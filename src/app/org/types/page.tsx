...
// The property-level types page uses useParams() to get a slug for its query key,
// but all DB queries are RLS-scoped and don't actually filter by slug.
// This wrapper renders the same UI without relying on a property slug param.
...
export default function OrgTypesPage() {
  ...
  async function handleAddType(e: React.FormEvent) {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['org', 'types'] });
      setNewName('');
      setNewIcon('📍');
      setNewColor('#5D7F3A');
                ...
                {activeTab === 'layout' && (
                  <LayoutBuilder
                    ...
                    onSave={async (layout, newFields) => {
                      ...
                      queryClient.invalidateQueries({ queryKey: ['org', 'types'] });
                    }}
